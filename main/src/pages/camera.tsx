...
const CameraComponent: React.FC = () => {
    ...
    return (
        <div className="h-screen max-w-lg m-auto flex flex-col justify-center items-center p-4 gap-2">
            {stream && <div className="flex-1 relative rounded-3xl">
                ...
                <div className={`absolute z-20 inset-0 flex justify-center items-center aspect-square rounded-3xl ${feedbackMessage && ' bg-neutral-900 bg-blend-color-dodge bg-opacity-80'}`}>
            ...
            {!stream &&
                <div className='p-4 gap-4 flex flex-col bg-neutral-900 rounded-3xl border-solid border-neutral-800 '>
                    <div className='flex flex-col gap-2'>
                        {/* <span className='rounded-3xl bg-neutral-800 w-fit p-2 font-bold text-xs'>v2.3beta</span> */}
                        ...
                        <p className='font-medium text-sm'>
                            It's very easy, just walk around your environment while waving your phone selfie camera up & down, left & right, far & near. Use your phone not your laptop if possible.
                        </p>
            ...
            {!!images.length && <div className='bg-neutral-900 p-4 w-full rounded-3xl flex flex-col justify-end gap-4 border-solid border-neutral-800 '>
                ...
                <div className={`min-h-[6rem] ${finished ? 'grid grid-cols-3 gap-2 ' : 'overflow-x-scroll overflow-y-hidden flex w-full gap-2 '} hide-scrollbar`}>
                    {images.map((image, i) => (
                        // <img key={i} className="h-full max-h-32 object-cover rounded-3xl" alt={`Captured frame ${i}`} src={URL.createObjectURL(image.image)} />
                        <div key={i} className='relative rounded-3xl w-24 min-w-[6rem] inline'
                            style={{
...
export const useCamera = (): UseCameraHook => {
    ...
    const captureImages = async () => {
        ...
        try {
            ...
            const clampedArrayToBlob = (clampedArray: Uint8ClampedArray) => {
                // Step 1: Convert Uint8ClampedArray to ArrayBuffer
                let arrayBuffer = new ArrayBuffer(clampedArray.length);
                let view = new Uint8Array(arrayBuffer);
                for (let i = 0; i < clampedArray.length; i++) {
                    view[i] = clampedArray[i];
                }
            ...
            while (true) {
                ...
                // const imageBlob = clampedArrayToBlob(imageData.data);
